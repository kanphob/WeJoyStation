...
/*:
 * @plugindesc (v.1.0) Trade (ANETZ Extension)
 * @author Pheonix KageDesu
 * @target MZ MV
 * @url https://gist.github.com/KageDesu/c6a2493994a75c3ad06fc54746758de0
 *
 * 
 * @help
 * ---------------------------------------------------------------------------
 ...
 *
 * GUIDE:
 * https://gist.github.com/KageDesu/c6a2493994a75c3ad06fc54746758de0
 *
 * ---------------------------------------------------------------------------
 *
 * Boosty:
 ...
 * License: Creative Commons 4.0 Attribution, Share Alike, Commercial
 * 
 * @param Alpha_NETZ_Ext_Trade
 * @text Alpha NET Z Extension - Trade
 * 
 * 
 ...
 * 
 * @param Alpha_NETZ_Ext_Trade
 * @text Alpha NET Z Расширение - Торговля
 * 
 * 
 ...
 * @param isTradeModalWindowActive:b
 * @type boolean
 * @text Нужно окно подтверждения обмена?
 * @on YES
 * @off NO
 * @default true
 * @desc Если ложь - обмен начнется немедленно для игрока, когда кто-то отправит ему запрос на обмен
 * 
 * 
 ...
 * @param tradeModalWindow_openSE
 * @parent isTradeModalWindowActive:b
 * @text Звук окна подтверждения
 * @type file
 * @require 1
 * @dir audio/se/
 * @default Bell3
 * @desc Звук, когда появляется окно подтверждения обмена
 * 
 * 
 ...
 * @param tradeModalWindow_text
 * @parent isTradeModalWindowActive:b
 * @text Текст запроса на обмен
 * @default Accept \C[1]Trade\C[0] from \C[2]%1\C[0]?
 * @desc Текст окна подтверждения обмена. %1 - имя другого игрока (который запросил обмен с вами).
 * 
 * 
 ...
 * @param tradeModalWindow_hkYes
 * @parent isTradeModalWindowActive:b
 * @text Горячая клавиша для "Да" 
 * @default y
 * @desc Горячая клавиша для ответа "Да" (с учетом регистра)
 * 
 * 
 ...
 * @param tradeModalWindow_hkNo
 * @parent isTradeModalWindowActive:b
 * @text Горячая клавиша для "Нет"
 * @default n
 * @desc Горячая клавиша для ответа "Нет" (с учетом регистра)
 * 
 * 
 ...
 * @off NO
 * @default true
 * @desc 如果为假 - 当有人向玩家发送交易请求时，交易将立即开始
 * 
 * 
 ...
 * @text 交易请求文本
 * @default Accept \C[1]Trade\C[0] from \C[2]%1\C[0]?
 * @desc 交易确认窗口文本。 %1 - 另一个玩家的名字（请求与你交易的人）。
 * 
 * 
...
var Imported;
(function (Imported) {
    Imported.Alpha_NETZ_Ext_Trade = true;
})(Imported || (Imported = {}));
var Alpha_NETZ_Ext_Trade;
(function (Alpha_NETZ_Ext_Trade) {
    Alpha_NETZ_Ext_Trade.Version = "1.0";
})(Alpha_NETZ_Ext_Trade || (Alpha_NETZ_Ext_Trade = {}));
...
(function (PP) {
    ...
    */
    function LoadPluginSettings() {
        _loader = new NZKDX.ParamLoader("Alpha_NETZ_Ext_Trade");
    }
    ...
    */
    ...
    function isShowModalForTradeRequest() {
        return getLoaderParam('isTradeModalWindowActive', true);
    }
    PP.isShowModalForTradeRequest = isShowModalForTradeRequest;
    function getTradeModalWindowText() {
        return getLoaderParam('tradeModalWindow_text', "Accept \\C[1]Trade\\C[0] from \\C[2]%1\\C[0]?");
    }
    PP.getTradeModalWindowText = getTradeModalWindowText;
    function getModalWindowOpenSE() {
        return getLoaderParam('tradeModalWindow_openSE', "Bell3");
    }
    PP.getModalWindowOpenSE = getModalWindowOpenSE;
    function getModalWindowYesHotkey() {
        return getLoaderParam('tradeModalWindow_hkYes', "y");
    }
    PP.getModalWindowYesHotkey = getModalWindowYesHotkey;
    function getModalWindowNoHotKey() {
        return getLoaderParam('tradeModalWindow_hkNo', "n");
    }
    PP.getModalWindowNoHotKey = getModalWindowNoHotKey;
})(PP || (PP = {}));
...
(function() {
  ...
  _.isHaveTradeParticipant = function() {
    return (this._hisId != null) && this._hisId !== "";
  };
  ...
  _.isClientAvailableForTrade = function() {
    return !this.isInTrade() && KDCore.Utils.isMapScene();
  };
  ...
  _.showTradeInDialog = function() {
    ...
    if (PP.isShowModalForTradeRequest()) {
  ...
  _.onTradeChange = function(item, count) {
    ...
    //LL.debug("Trade items [my] changed")
  ...
  _.onTradeShouldComplete = function() {
    //LL.debug("TRADE SHOULD BE COMPLETED")
  ...
  _.sendStartTradeRequest = function(anotherPlayer) {
    ...
    ANNetwork.send(NetMessage.Common("trade_request", data));
    ANNetwork.setWait();
  ...
  _.sendRefuseTradeRequest = function(anotherPlayerId) {
    ANNetwork.send(NetMessage.Common("trade_refuse", anotherPlayerId));
  };
  _.sendAcceptTradeRequest = function(anotherPlayerId) {
    ANNetwork.send(NetMessage.Common("trade_accept", anotherPlayerId));
  };
  _.sendMyTradeItems = function() {
    var data, items;
    items = $gameTemp._netMyTradeItems.map(function(item) {
      if (item != null) {
        return [ANETUtils.packItemForNetwork(item[0]), item[1]];
      } else {
        return null;
      }
    });
    ...
    ANNetwork.send(NetMessage.Common("trade_items", data));
  };
  _.sendMyTradeReadyStatus = function(status) {
    ...
    ANNetwork.send(NetMessage.Common("trade_status", data));
  };
  _.sendCompleteTrade = function() {
    ANNetwork.callback(NetMessage.Common("trade_complete", ANNetwork.myId()), function() {
  ...
  _.onTradeCompleted = function() {
    ...
    try {
      //KAudio.PlaySE(PP.tradeSceneCompleteSE())
      // * Забираем вещи у игрока (которые он предлагал)
      //LL.debug("TRADE COMPLETED")
  ...
  // * Обработка общей команды NetMessage.Common
  ...
  _._on_trade_items = function(content) {
    ...
    try {
      ...
      return $gameTemp._netHisNetTradeItems = items.map(function(item) {
        if (item != null) {
          return [ANETUtils.unpackItemFromNetwork(item[0]), item[1]];
        } else {
          return null;
        }
      });
...
//---------------------------------------------------------------------------
(function() {
  var ALIAS___getCurrentSceneType, ALIAS__onMapLoaded, _;
  //@[DEFINES]
  _ = ANGameManager;
  //@[ALIAS]
  ...
  _.onMapLoaded = function() {
    ...
    // * Сбрасываем торговлю
    return ANTradeManager.onTradeSceneEnd();
  };
})();
...
FWindow_InGameModalDialog = class FWindow_InGameModalDialog extends KDCore.FloatingWindow {
  ...
  setTimer(seconds) {
    if (seconds <= 0) {
      ...
      this._timerThread = new NZKDX.TimedUpdate(1, this._tick.bind(this));
    }
  }
...
(function() {
  ...
  _._createDialogButtons = function() {
    var margin;
    margin = 16;
    this.btnYes = new KDCore.ButtonM("nzModalYesButton", false, "Alpha_NETZ_Ext_Trade");
    this.btnYes.addClickHandler(this._onBtnYesClick.bind(this));
    this.btnYes.move(margin, 62);
    this.addContent(this.btnYes);
    this.btnNo = new KDCore.ButtonM("nzModalNoButton", false, "Alpha_NETZ_Ext_Trade");
  ...
  _._updateHotkeys = function() {
    if (Input.isTriggered(PP.getModalWindowYesHotkey())) {
      this._onBtnYesClick();
    } else if (Input.isTriggered(PP.getModalWindowNoHotKey())) {
      this._onBtnNoClick();
    }
  };
})();
...
// Generated by CoffeeScript 2.7.0
var FloatingWindowAlt;

FloatingWindowAlt = class FloatingWindowAlt extends KDCore.Sprite {
  constructor(mainParent, windowW, windowH, parameters) {
    super();
    this.mainParent = mainParent;
    this.windowW = windowW;
  ...
  static StaticWindow(parent, sub) {
    var p, w;
    p = FloatingWindowAlt.StaticSettings();
    w = new FloatingWindowAlt(parent, sub.width, sub.height, p);
    w.setSubWindow(sub);
    w.open();
    return w;
  }
...
(function() {
  ...
  //@[DEFINES]
  _ = ImageManager;
  _.loadNZETrade = function(filename) {
    return this.loadBitmap('img/Alpha_NETZ_Ext_Trade/', filename);
  };
})();
...
//---------------------------------------------------------------------------
(function() {
  var ALIAS__event_common_forClients, _;
  //@[DEFINES]
  _ = NetClientMethodsManager;
  //@[ALIAS]
  ALIAS__event_common_forClients = _.event_common_forClients;
  _.event_common_forClients = function(content) {
    var cmd, data, e;
    try {
      ({cmd, data} = content);
      if (cmd.contains("trade")) {
        ANTradeManager.onCommonCommandFromServer(cmd, data);
        return;
      }
    } catch (error) {
      e = error;
    ...
    ALIAS__event_common_forClients.call(this, ...arguments);
  };
})();
...
(() => {
    ...
    _.start = function () {
        ...
        if (!Imported['Alpha_NETZ']) {
            ...
            window.alert("Trade Extension plugin require Alpha NETZ!");
            return;
        }
        PP.LoadPluginSettings();
        ANET.registerExtension("Trade");
    };
})();
// ■ END Scene_Boot.ts
//---------------------------------------------------------------------------
...
Scene_NetworkInGameTrade = class Scene_NetworkInGameTrade extends Scene_MenuBase {
  ...
  create() {
    ...
    this.checkAnotherPlayerSceneStateThread = new NZKDX.TimedUpdate(30, this._checkAnotherPlayerSceneStateTick.bind(this));
  }
...
(function() {
  ...
  _._preloadResources = function() {
    //TODO: from parameters
    ImageManager.loadNZETrade("nzTradeButton_00");
    ImageManager.loadNZETrade("nzTradeButton_01");
    ImageManager.loadNZETrade("nzTradeStatus_NotReady");
    ImageManager.loadNZETrade("nzTradeStatus_Ready");
  };
  ...
  _._createAndAddImage = function(item) {
    ...
    img = new KDCore.UI.Sprite_UIImage({
      visible: true,
      image: item.image,
      rootImageFolder: "Alpha_NETZ_Ext_Trade"
    });
  ...
  _._createTradeWindowA = function() {
    ...
    this.tradeWindowA = FloatingWindowAlt.StaticWindow(this, tradeListWindow);
  ...
  _._createTradeWindowB = function() {
    ...
    tradeListWindow.refresh();
    this.tradeWindowB = FloatingWindowAlt.StaticWindow(this, tradeListWindow);
...
(function() {
  ...
  _._drawTradeCommand = function() {
    ...
    try {
      if (this.isOwnerWindow()) {
        if (this.index() === index) {
          b = ImageManager.loadNZETrade("nzTradeButton_01");
        } else {
          b = ImageManager.loadNZETrade("nzTradeButton_00");
        }
      } else {
        if (this.isEnabled(index)) { // * Player B is ready
          b = ImageManager.loadNZETrade("nzTradeStatus_Ready");
        } else {
          b = ImageManager.loadNZETrade("nzTradeStatus_NotReady");
        }
      }
      b.addLoadListener(() => {
...
//---------------------------------------------------------------------------
(function() {
  var ALIAS__packItemForNetwork, ALIAS__unpackItemFromNetwork, _;
  //@[DEFINES]
  _ = ANETUtils;
  //@[ALIAS]
  ALIAS__packItemForNetwork = _.packItemForNetwork;
  _.packItemForNetwork = function(item) {
    if (item == null) {
      return null;
    }
    if (item.id === ANTradeManager.TradeGoldItemId) {
      return {
        id: item.id,
        typeId: 0
      };
    }
    return ALIAS__packItemForNetwork.call(this, ...arguments);
  };
  
  //@[ALIAS]
  ALIAS__unpackItemFromNetwork = _.unpackItemFromNetwork;
  _.unpackItemFromNetwork = function(data) {
    if (data == null) {
      return null;
    }
    if (data.id === ANTradeManager.TradeGoldItemId) {
      return {
        id: data.id
      };
    }
    return ALIAS__unpackItemFromNetwork.call(this, ...arguments);
  };
})();

// ■ END ANET.Utils.coffee
//---------------------------------------------------------------------------
...
(function() {
  ...
  _.terminate = function() {
    ...
    return this.terminateModalWindow();
  };
  (function() {    // * Модальное диалоговое окно
    // -----------------------------------------------------------------------
