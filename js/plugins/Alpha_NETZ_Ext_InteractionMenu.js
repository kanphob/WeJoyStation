...
/*:
 * @plugindesc (v.1.0) Interaction Menu (ANETZ Extension)
 * @author Pheonix KageDesu
 * @target MZ MV
 * @url https://gist.github.com/KageDesu/d741e0fc452c7af2da88fe00e4923fdb
 *
 * 
 * @help
 * ---------------------------------------------------------------------------
 ...
 *
 * GUIDE:
 * https://gist.github.com/KageDesu/d741e0fc452c7af2da88fe00e4923fdb
 *
 * ---------------------------------------------------------------------------
 *
 * Boosty:
 ...
 * License: Creative Commons 4.0 Attribution, Share Alike, Commercial
 * 
 * @param Alpha_NETZ_Ext_InteractionMenu
 * @text Alpha NET Z Extension - Interaction Menu
 * 
 * 
 ...
 * 
 * @param Alpha_NETZ_Ext_InteractionMenu
 * @text Alpha NET Z Расширение - Интерактивное меню
 * 
 * 
 ...
 * @text Обмен
 * @type struct<LPlayerMenuOption> 
 * @desc Отправить запрос на обмен другому игроку
 * @default {"text:str":"Trade","visible:b":"true","switchId:i":"0","minDist:i":"1","value:i":"0"}
 * 
 * 
 ...
 * @text Статус
 * @type struct<LPlayerMenuOption> 
 * @desc Показать экран состояния другого персонажа игрока
 * @default {"text:str":"Status","visible:b":"true","switchId:i":"0","minDist:i":"0","value:i":"0"}
 * 
 * 
 ...
 * @text Следовать
 * @type struct<LPlayerMenuOption> 
 * @desc Начать следовать за другим персонажем игрока на карте
 * @default {"text:str":"Follow","visible:b":"true","switchId:i":"0","minDist:i":"0","value:i":"0"}
 * 
 * 
 ...
 * @type struct<LPlayerMenuOption>[]
 * @desc Ваши команды для меню игрока
 * @default []
 * 
 * 
...
@text Заголовок
@type text
@desc Заголовок опции. Поддерживает управляющие символы, такие как \C[x] для цвета
@default Option
...
@on Yes
@off No
@desc Видима ли эта опция в меню игрока? Если нет - не появляется в меню вообще
@default true
...
@type switch
@desc ID переключателя для включения этой опции. 0 - всегда включено, без переключателя
@default 0
...
@param minDist:i
@text Минимальная дистанция
@type number
@min 0
@max 100
@desc Минимальная дистанция между игроками для включения этой опции. 0 - любая дистанция, без ограничений
@default 0
...
@text Общие события
@type common_event
@desc ID общего события для этой команды
@default 0
*/

/*~struct~LPlayerMenuOption:zh-ch

@param text:str
@text 标题
@type text
@desc 选项标题。支持控制字符，如\C[x]表示颜色
@default Option
...
@on Yes
@off No
@desc 此选项在玩家菜单中是否可见？如果为否 - 则完全不出现在菜单中
@default true
...
@min 0
@max 100
@desc 启用此选项的玩家之间的最小距离。0 - 任何距离，无限制
@default 0
...
var Imported;
(function (Imported) {
    Imported.Alpha_NETZ_Ext_InteractionMenu = true;
})(Imported || (Imported = {}));
var Alpha_NETZ_Ext_InteractionMenu;
(function (Alpha_NETZ_Ext_InteractionMenu) {
    Alpha_NETZ_Ext_InteractionMenu.Version = "1.0";
})(Alpha_NETZ_Ext_InteractionMenu || (Alpha_NETZ_Ext_InteractionMenu = {}));
...
(function (PP) {
    ...
    */
    function LoadPluginSettings() {
        _loader = new NZKDX.ParamLoader("Alpha_NETZ_Ext_InteractionMenu");
    }
    ...
    */
    ...
    function getPlayerMenuDefaultCommands() {
        if (!menuCommands) {
            menuCommands = [];
            const commandNames = ["trade", "status", "follow"];
            for (const name of commandNames) {
                const cmd = getLoaderParam("PlayerMenuItem_" + name, null);
                if (cmd && cmd.visible === true) {
                    cmd.action = name; // * flag
    ...
    PP.getPlayerMenuDefaultCommands = getPlayerMenuDefaultCommands;
    function getPlayerMenuUserCommands() {
        return getLoaderParam('userIPLMenuCommands', []);
    }
    PP.getPlayerMenuUserCommands = getPlayerMenuUserCommands;
})(PP || (PP = {}));
...
FWindow_InGamePlayerMenu = class FWindow_InGamePlayerMenu extends KDCore.FloatingWindow {
  ...
  rootImageFolder() {
    return "Alpha_NETZ_Ext_InteractionMenu";
  }
...
(function() {
  ...
  _._executeMenuItem = function(item) {
    ...
    try {
      if (item.action && item.action !== "") { // * Только для стандартных комманд
...
(function() {
  ...
  //@[DEFINES]
  _ = ImageManager;
  _.loadNZEInteractionMenu = function(filename) {
    return this.loadBitmap('img/Alpha_NETZ_Ext_InteractionMenu/', filename);
  };
})();
...
(() => {
    ...
    _.start = function () {
        ...
        if (!Imported['Alpha_NETZ']) {
            ...
            window.alert("Interaction Menu Extension plugin require Alpha NETZ!");
            return;
        }
        PP.LoadPluginSettings();
        ANET.registerExtension("Interaction Menu");
    };
})();
// ■ END Scene_Boot.ts
//---------------------------------------------------------------------------
...
(function() {
  ...
  Sprite_MapClickEffect = class Sprite_MapClickEffect extends Sprite {
    constructor(x, y) {
      super();
      this.bitmap = ImageManager.loadNZEInteractionMenu("nzClickEffect");
      this.anchor.set(0.5);
      this._create();
      this.move(x, y);
      return;
...
(function() {
  ...
  _._create = function() {
    ...
    return this._createInGamePlayerMenu();
  };
})();
...
Window_NPlayerCommandsList = class Window_NPlayerCommandsList extends Window_Selectable {
  ...
  isEnabled(index) {
    ...
    switch (item.action) {
      case "trade":
        if (!nAPI.isExtensionLoaded("Alpha_NETZ_Ext_Trade")) {
          return false;
        }
        if (this.char != null) {
...
(function() {
  ...
  _._prepareCommandsForCurrentChar = function() {
    ...
    this._items = [];
    defCommands = PP.getPlayerMenuDefaultCommands();
    for (i = 0, len = defCommands.length; i < len; i++) {
      c = defCommands[i];
      this._items.push(c);
    }
    userCommands = PP.getPlayerMenuUserCommands();
      ...
      try {
        switch (actionName) {
          case "trade":
            if (!nAPI.isExtensionLoaded("Alpha_NETZ_Ext_Trade")) {
              SoundManager.playBuzzer();
              console.warn("Trade Extension is not loaded!");
              return false;
            }
          ...
          default:
            return console.warn("Unknown Player Menu Action: " + actionName);
        }
      } catch (error) {
        e = error;
        console.warn(e);
...
//---------------------------------------------------------------------------
(function() {
  var ALIAS__onGameMessageStart, _;
  //@[DEFINES]
  _ = ANET.UI;
  //@[ALIAS]
  ALIAS__onGameMessageStart = _.onGameMessageStart;
  _.onGameMessageStart = function() {
    ALIAS__onGameMessageStart.call(this, ...arguments);
    return this.closePlayerMenu();
  };
  (function() {    // * Меню игрока
    // -----------------------------------------------------------------------
  ...
  _.playClickEffect = function(noSound = false) {
    var effect, x, y;
    if (!this.isValid()) {
      return;
    }
